...
export function getSize({ size }: Props): string {
    if (typeof size === 'number') return `${size}px`;
    if (typeof size === 'string' && isLegalCSSLengthUnit(size)) return size;
    return '16px';
}

export function getColor({ color }: Props): string {
    if (typeof color === 'string' && isLegalCSSColor(color)) return color;
    return '#3E4DA7';
}

export function getMargin({ labelPlacement, interactive, labelDistance }: Props): string {
    let distance;
...
export function getFlexDirection({ labelPlacement }: Props): string {
