export function addNode(data, targetId, newNode) {
...
export function deleteNode(data, targetId) {
...
export function addBranch(data, targetId, newNodes) {
    const preNode = data[targetId];
...
export const getCombineNode = (data, branchNodes, duplicate) => { // 11-寻找收束节点的稳定的方法
    const flag = [];
    ...
    if (duplicate[flag[0].id]) return null; // 12-多分支共享同一个收束节点标记
    duplicate[flag[0].id] = true;// 12-多分支共享同一个收束节点标记
    return data[flag[0].id];
}

export function expandBranch(data, targetId, newNodes) {
    const preNode = data[targetId];
    const duplicate = {};
    const combineNode = getCombineNode(data, preNode.next.map(nextId => data[nextId]), duplicate);
    newNodes.forEach(node => node.pre = [preNode.id])
...
export function combineNodes(data, targetIds, newNode) {
    const duplicate = {}; // 12-多分支共享同一个收束节点标记
    const combineNode = getCombineNode(data, targetIds.map(id => data[id]), duplicate);
    const nodesNeedCombine = [];
